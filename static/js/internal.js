async function updateButton(buttonId, maxAttempts) {
  ...
  const response = await fetch(
    `/files/update_button/${buttonId}?max_attempts=${maxAttemptsInt}`,
    {
      method: "POST",
      headers: {
...
async function ButtonStatus(buttonId) {
  ...
  const maxAttemptsElement = document.getElementById(
    `maxAttemptsInput_${buttonId}`,
  );
...
function showError(message) {
  ...
  console.log("[SHOW_ERROR] Показываем ошибку:", message);
